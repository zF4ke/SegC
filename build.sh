...
jar --create --file build/libs/mySharingServer.jar --manifest build/manifest/server_manifest.mf -C build .
jar --create --file build/libs/mySharingClient.jar --manifest build/manifest/client_manifest.mf -C build .

echo "Build concluído! JARs estão em build/libs"